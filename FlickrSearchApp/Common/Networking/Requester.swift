...
class Requester {
    ...
    static func requestJSON(path: String, method: String, parameters: [String: Any], placeholders: [String: CustomStringConvertible], mapper:((Any) -> Any)?, completion:@escaping (Any?, Error?) -> ()) -> Cancallable {
    ...
    static func requestData(path: String, method: String, parameters: [String: Any], placeholders: [String: CustomStringConvertible], completion:@escaping (Any?, Error?) -> ()) -> Cancallable {
        let path = PathMaker.makePath(for: path, placeholders: placeholders)
        return Alamofire.request(path, method: self.method(by: method), parameters: parameters).validate(statusCode: 200..<300).responseData { (response) in
